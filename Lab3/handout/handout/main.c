...
int main()
{
	...
	if (counter == 0)
		...
		printf("Error in array creation\n");

	for (int i = 0; i < size; i += 2)
	{
		add(arr, i, counter);
		counter++; // Implemented incrementing counter
	...
	if (contains(arr, size, 6))
		...
		printf("Number 6 not in Array Contains not working properly\n");

	if (contains(arr, size, 30))
		printf("Number 30 present in Array Contains not working properly\n");
	else
		printf("Number 30 not in Array\n");
